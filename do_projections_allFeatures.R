...
{
  cat('\n\nProcessing:', spName[taxon], 'on fold', k-1, '\n\n')
  # lambdas.file <- strsplit(x=fit@models[[k]]@lambdas,split=',',fixed=TRUE)
  setwd('/curie_data/ncastaneda/gap-analysis/gap_avena/maxent_modeling/models/Avena_abyssinica/crossval/')
  lambdas.file <- readLines(paste(spName[taxon], '_', k-1, '.lambdas', sep='')) # Read lambdas file by fold
